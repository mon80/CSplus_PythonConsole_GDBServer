# GDBSERVER on CubeSuite+'s Python Console (for Renesas RX Simulator)
# coding=utf-8

#       Created on: May 05, 2012
# Last Modified on: Feb 26, 2013
#           Author: MON-80
#          License: MIT
...
#       CPU使用率が100%に張り付く問題があります
#       動的に自分自身のスクリプト内の関数/変数の定義をアップデートすることが出来ません
#       RXシミュレータではCubeSuiteW+.exe終了後もCubeSuiteW+.exeのプロセスが残る問題があります
#       (e2studioから外部ツールとして起動した場合にはe2studioからプロセスを強制終了出来ます)
...
#   monitor load Elf/Dwarf2 ファイル名.lmf
#     CubeSuite+のデバッグツール上でElf/Dwarf2ファイルをダウンロードします
#     CubeSuite+がElf/Dwarf2ファイルと認識するものであればファイルの拡張子は何でも構いません
...
#
# TODO:
# RXシミュレータのPythonオブジェクトではアクセス系ブレークポイントアドレスを即値で設定出来ない
# (シンボルで設定しなければならない。他方、実行系ブレークポイントアドレスは即値で設定出来る。)
...
VERSION = "v1.01.01.00" # 2013/02/26
SVNAME = "CubeSuite+'s Python Console"
...
RegAll = range(0, 0x1a)
RegAtBrk = range(0x13,0x14) + range(0x10,0x12) # pc, usp, isp
RegExcl = ""
...
def RegGetVal_(reg):
    ...
    try:
        ...
        val = int(debugger.Register.GetValue(reg))
...
# TODO: RXシミュレータのPythonオブジェクトではアクセス系ブレークポイントアドレスを即値で設定出来ない
# (シンボルで設定しなければならない。他方、実行系ブレークポイントアドレスは即値で設定出来る。)
# TODO: エラー発生時はどうする?
...
#    DebReset()
    while True:
        ...
        if cmd[0:1] == "":
            ...
            # RXではレジスタ値をGDBへ送り返す時の個々のDataのバイトオーダーはエンディアン設定依存
            SendPacket(RegGetValAll())
#        # 無くても動いたのでコメントアウトしておく(デバッグに便利だったのでコードは残しておく)
#        elif cmd[0:1] == "p":
#            # Read a Register : 'p' + <Id[1,2桁]>
#            # RXではレジスタ値をGDBへ送り返す時のDataのバイトオーダーはエンディアン設定依存
#            id, i = HexStrToVal(cmd[1:], "", True)
#            SendPacket(RegGetVal(id))
        elif cmd[0:1] == "P":
            # Write a Register : 'P' + <Id[1,2桁]> + '=' + <Data[可変桁(RXではバイトオーダーはエンディアン設定依存)]>
            id, i = HexStrToVal(cmd[1:], "=", True)
